...
* If the [VerbAttribute](./VerbAttribute.cs) is created without the handler type parameter, it will default to use [HelpCommandAction](./HelpCommandAction.cs)instead of [DefaultCommandAction](./DefaultCommandAction.cs).
...
* The [VerbAttribute](./VerbAttribute.cs) can now be created without the handler type parameter.  The system will use the [DefaultCommandAction](./DefaultCommandAction.cs).
* New sub command support.  See [Sub Commands](../docs/sub-commands.md).
* Create [HelpCommandAction](./HelpCommandAction.cs) that will display the help message of a command
...
* If a command has its own handler, it will no longer be overwritten with the [GlobalCommandAction](./GlobalCommandAction.cs).  This gives developers more flexibility in creating custom commands.
