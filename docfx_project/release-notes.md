...
- **System.CommandLine v3 & multi-targeting** - core migrates to the v3 prerelease and now multi-targets `netstandard2.1;net10.0`, tracking System.CommandLine's own target frameworks (a netstandard floor for reach plus a modern `net10.0` leg). The `net10.0` leg is AOT/trim-analyzed (`IsAotCompatible`). `Microsoft.Extensions.Hosting`/`DependencyInjection` move to **10.x** - the earlier hold-at-8.0.1 was dropped now that .NET 8/9 are EOL/near-EOL and the ecosystem is on 10.
- **Source generator bundled into the core package** - the Roslyn generator now ships *inside* `Albatross.CommandLine` (under `analyzers/dotnet/cs`), so referencing the core package auto-activates code generation. `Albatross.CommandLine.CodeGen` is no longer a separate package to reference. (`Albatross.CommandLine.CodeAnalysis` remains an opt-in analyzer package you add explicitly.)
