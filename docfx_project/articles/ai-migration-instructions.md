...
> **v9 change:** the generated `RegisterCommands()` and `AddCommands()` extension methods are now emitted into the `<RootNamespace>.AutoGenerated` namespace (previously they followed the entry point's namespace). Add a `using <RootNamespace>.AutoGenerated;` to `Program.cs` — and to any other file that calls them — replacing `<RootNamespace>` with your project's root namespace (defaults to the assembly name).
