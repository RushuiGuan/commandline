...
1. Services Registration method - A static service registration method that can be used to register the parameters class using scoped lifetime and the command handler by keyed scoped lifetime.  Notice that the instance of the `parameters` class is created by the service registration code using initializers.  The `CodeGenExtensions` class that hosts `RegisterCommands()` and `AddCommands()` is generated into the `<RootNamespace>.AutoGenerated` namespace (falling back to `<AssemblyName>.AutoGenerated` when no `RootNamespace` is set), so add a `using` for that namespace wherever you call these methods.
```csharp
...
```csharp
using MyApp.AutoGenerated;   // RegisterCommands()/AddCommands() are generated in <RootNamespace>.AutoGenerated
